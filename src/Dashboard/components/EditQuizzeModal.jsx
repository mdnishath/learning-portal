...
const EditQuizzeModal = ({ quizzeId }) => {
  const dispatch = useDispatch();
  const [change, setChange] = useState(false);
  const { isEditing } = useSelector((state) => state.quizzes);
  ...
  //dist obj
  const { question, video_title, options } = formData || {};
  ...
  } else if (!isLoading && !isError && quiz && Object.keys(quiz).length > 0) {
    content = (
...
export default EditQuizzeModal;
