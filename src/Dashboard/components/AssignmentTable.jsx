...
import { chandgeEditStutas } from "../../features/assignments/assignmentsSlice";
...
const AssignmentTable = ({ assignments }) => {
  ...
  const handleDelete = (id) => {
    dispatch(assignmentsApi.endpoints.removeAssignment.initiate(id));
  };

  return (
    <>
      <EditAssignmentModal assignmentId={assignmentId} />
        ...
        <tbody className="divide-y divide-slate-600/50">
          {assignments.map((assignment) => (
            <tr key={assignment?.id}>
              ...
              <td className="table-td whitespace-normal flex gap-x-2">
                ...
                <button onClick={() => handleUpdate(assignment?.id)}>
                  <svg
                    fill="none"
                    viewBox="0 0 24 24"
                    strokeWidth="1.5"
